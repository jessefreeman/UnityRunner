...
public class SpriteStressDemoRunner : BaseRunner {
	...
	public override List<string> defaultChips
	{
		get
		{
			var chips = base.defaultChips;
				...
				chips.Add(typeof(SpriteStressDemoChip).FullName);
...
public class SpriteStressDemoChip : GameChip
{
...
// the background color.
	public override void Update(float timeDelta)
	{
		...
		if (delay > delayTime)
		{
			...
			// between 0 and 63.
			BackgroundColor(Random.RandomRange(0, totalColors));
...
// is where all of our draw calls should go. We'll be using this to render sprites to the display.
	...
	// we can directly clear the DisplayChip by calling the Clear() method.
		...
		{
			x = Random.RandomRange(0, width);
			y = Random.RandomRange(0, height);
			DrawSprite(i + 12, x, y);
